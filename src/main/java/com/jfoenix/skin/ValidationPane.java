...
import com.jfoenix.control.base.IFXStaticControl;
import com.jfoenix.control.base.IFXValidatableControl;
import com.jfoenix.util.JFXUtil;
    ...
    public ValidationPane(T control) {
        ...
        control.activeValidatorProperty().addListener((ObservableValue<? extends ValidatorBase> o, ValidatorBase oldVal, ValidatorBase newVal) -> {
            if (!control.isDisableAnimation()) {
                ...
                if (newVal != null) {
                    JFXUtil.runInFXAndWait(() -> showError(newVal));
                } else {
                    JFXUtil.runInFXAndWait(this::hideError);
                }
            }
        });
    }
